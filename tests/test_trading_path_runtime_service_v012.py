...
def test_runtime_oos_changes_cannot_change_pre_oos_pipeline(monkeypatch):
    ...
    monkeypatch.setattr(
        "edward.services.conditional_discovery_service_v086.ConditionalDiscoveryServiceV086.run",
        lambda candles: discovery_inputs.append(tuple(candles)) or SimpleNamespace(evidence=()),
    )
    monkeypatch.setattr(
        "edward.services.trading_path_adaptive_discovery_service_v014.TradingPathAdaptiveDiscoveryServiceV014.run",
        lambda candles: adaptive_inputs.append(tuple(candles)) or SimpleNamespace(candidates=()),
    )
    monkeypatch.setattr(
        "edward.services.trading_path_candidate_service_v014.TradingPathCandidateServiceV014.from_fixed",
        lambda discovery, **kwargs: (candidate,),
    ...
    monkeypatch.setattr(
        "edward.services.trading_path_candidate_service_v014.TradingPathCandidateServiceV014.combine",
        lambda fixed, adaptive, **kwargs: tuple(fixed) + tuple(adaptive),
    )
    monkeypatch.setattr(
        "edward.services.trading_path_candidate_pruning_service_v014.TradingPathCandidatePruningServiceV014.prune",
        lambda candidates, **kwargs: tuple(candidates),
    ...
    first_discovery = tuple(discovery_inputs)
    first_adaptive = tuple(adaptive_inputs)
    first_validation = tuple(validation_inputs)

    discovery_inputs.clear()
    adaptive_inputs.clear()
    validation_inputs.clear()
    runtime.analyze_paths(instrument_uid="SBER", ticker="SBER", candles=_candles(oos_close=150.0))

    assert discovery_inputs == list(first_discovery)
    assert adaptive_inputs == list(first_adaptive)
    assert validation_inputs == list(first_validation)
