...
def test_runtime_runs_canonical_path_stages(monkeypatch):
    ...
    monkeypatch.setattr("edward.services.trading_path_oos_validation_service_v012.TradingPathOOSValidationServiceV012.validate", lambda candidate, candles: ())
    monkeypatch.setattr("edward.services.trading_path_expected_value_service_v012.TradingPathExpectedValueServiceV012.calculate", lambda candidate, candles: SimpleNamespace(expected_value_pct=1.0, edge_reliability_pct=80.0))
