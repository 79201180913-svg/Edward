...
def candles_from_closes(closes: list[float], *, start: datetime | None = None) -> list[Candle]:
...
def test_point_in_time_validation_passes_when_future_is_appended():
    ...
    base_candles = candles_from_closes(base)
    future_candles = candles_from_closes(future, start=base_candles[-1].timestamp + timedelta(days=1))
    result = PointInTimeValidationService.validate_all(base_candles, future_candles=future_candles)
...
def test_forecast_validation_detects_changed_origin_input():
    ...
    assert PointInTimeValidationService.validate_forecast(base, future_candles=[changed]) is False


def test_validation_reports_layer_failure():
    base = candles_from_closes([100.0 * (1.001 ** i) for i in range(120)])
    base_candles = base
    future_candles = candles_from_closes(
        [2000.0 * (1.05 ** i) for i in range(100)],
        start=base_candles[-1].timestamp + timedelta(days=1),
    )
    result = PointInTimeValidationService.validate_all(base_candles, future_candles=future_candles)
