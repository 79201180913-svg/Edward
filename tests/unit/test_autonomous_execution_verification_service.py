...
def test_buy_is_rejected_when_position_not_reached():
    result = AutonomousExecutionVerificationService().verify(
        step=step(), state=state(5), expected_quantity=10,
    )
...
def test_sell_requires_zero_position():
    result = AutonomousExecutionVerificationService().verify(
        step=step("SELL"), state=state(0), expected_quantity=0,
    )
    assert result.passed is True


def test_reduce_requires_lower_quantity():
    result = AutonomousExecutionVerificationService().verify(
        step=step("REDUCE"), state=state(4), expected_quantity=4, before_quantity=10,
    )
    assert result.passed is True


def test_reduce_is_rejected_when_quantity_did_not_decrease():
    result = AutonomousExecutionVerificationService().verify(
        step=step("REDUCE"), state=state(10), expected_quantity=4, before_quantity=10,
    )
    assert result.passed is False
    assert "POSITION_NOT_REDUCED" in result.reasons[0]
