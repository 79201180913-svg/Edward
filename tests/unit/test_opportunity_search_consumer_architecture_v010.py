...
def test_live_opportunity_search_uses_canonical_analysis_consumer():
    source = _source()

    assert "OpportunityAnalysisConsumerV010.from_result" in source
    assert "self.analysis_pipeline.analyze(" in source
    assert "self._from_canonical_result(" in source
