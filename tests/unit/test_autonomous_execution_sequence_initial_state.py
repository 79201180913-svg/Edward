...
class FakeBridge:
    ...
    def enqueue_opportunity(self, *, account_id, result):
        ...
        return SimpleNamespace(
            accepted=True,
            request=SimpleNamespace(execution_id=f"exec-{self.counter}", quantity=Decimal("10")),
            result=SimpleNamespace(status=ExecutionStatus.READY),
            reason="",
        )
...
class FakeStepService:
    def prepare_step(self, *, account_id, step, result, dependency_completed):
        return SimpleNamespace(
            accepted=True,
            request=SimpleNamespace(execution_id="exec-1", quantity=Decimal("10")),
            result=SimpleNamespace(status=ExecutionStatus.READY),
            reason="",
        )
...
def test_initial_state_is_used_without_pre_execution_refresh():
    ...
    plan = AutonomousExecutionPlan(steps=(ExecutionPlanStep(
        sequence=1,
        action="BUY",
        ticker="NEW",
        instrument_uid="new",
    ...
    outcome = service.execute_confirmed_plan(
        account_id="ACC",
        plan=plan,
        result_factory=lambda step: SimpleNamespace(recommended_quantity=10),
        initial_state=AccountState(portfolio=None, positions=[], balance=None, orders=[]),
    )

    assert outcome.completed is True
    # One refresh remains mandatory: verification after the broker fill.
