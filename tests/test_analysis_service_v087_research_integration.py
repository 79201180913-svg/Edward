...
def test_v087_research_fields_do_not_change_empty_recommendation(monkeypatch):
    ...
    # This test exercises the public diagnostics contract while replacing the expensive
    # production strategy/WF execution with deterministic stubs.
    ...
    monkeypatch.setattr(
        "edward.services.analysis_service_v08.ResearchEvidenceReportServiceV086.build",
        lambda evidence, wf_evidence: (),
    )
    monkeypatch.setattr(
        "edward.services.analysis_service_v08.ResearchEvidenceSummaryServiceV087.build",
        lambda rows: SimpleNamespace(total_cells=0, interesting=0, low_sample=0, no_positive_excess=0, low_wf_persistence=0, top_magnitude=(), top_consistency=(), top_stability=()),
    )
    monkeypatch.setattr(
        service,
        "_robust",
        lambda ordered, strategy, profile: SimpleNamespace(strategy=strategy, windows=(), robustness_score=0.0, mean_test_return_pct=0.0, mean_test_drawdown_pct=0.0, mean_test_sharpe=0.0, positive_return_windows=0, risk_ok_windows=0, positive_sharpe_windows=0, return_consistency_pct=0.0, risk_consistency_pct=0.0, sharpe_consistency_pct=0.0),
    )
    monkeypatch.setattr(
