...
def test_v086_conditional_discovery_is_exposed_only_in_diagnostics():
    ...
    result = service.analyze(instrument_uid="V086-001", ticker="TEST", candles=_candles(), profile="medium_term")
    ...
    assert diagnostics.evidence_audit
    assert len(diagnostics.evidence_audit) > 0
    assert all(item.observations >= 0 for item in diagnostics.evidence_audit)
...
def test_v086_conditional_discovery_cannot_bypass_quality_gate():
    service = AnalysisServiceV08()
    result = service.analyze(instrument_uid="V086-002", ticker="TEST", candles=_candles(), profile="medium_term")
...
def test_v086_conditional_discovery_does_not_change_strategy_set():
    service = AnalysisServiceV08()
    result = service.analyze(instrument_uid="V086-003", ticker="TEST", candles=_candles(), profile="medium_term")
