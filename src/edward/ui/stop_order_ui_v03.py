...
def install_stop_order_ui(app_class: type[Any]) -> None:
    if getattr(app_class, "_stop_order_ui_v03_installed", False):
        return
    ...
    def _render_protection(app: Any) -> None:
        ...
        if position is None or _decimal(_field(position, "balance", 0)) == 0:
            ttk.Label(
                protection,
                text="Открытой позиции нет. Stop Loss и Take Profit доступны после покупки.",
            ).pack(anchor="w")
        else:
        ...
        try:
            ...
            orders = _items(response, "stop_orders")
        except Exception as exc:
            ttk.Label(protection, text=f"Не удалось получить список: {exc}").pack(anchor="w")
            return

        if not orders:
            ttk.Label(protection, text="Нет активных защитных заявок").pack(anchor="w")
            return
    ...
    app_class._stop_order_ui_v03_installed = True
