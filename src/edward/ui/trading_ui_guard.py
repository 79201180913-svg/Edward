...
def install_trading_ui_guard(app_class: type[Any]) -> None:
    ...
    def _show_trading_unavailable(self: Any, error: Exception | None = None) -> None:
        ...
        ttk.Label(self.content, text="Торговля недоступна", style="Title.TLabel").pack(anchor="w", pady=(0, 12))
        ttk.Label(
            self.content,
            text=f"Инструмент: {ticker}\nДля выбранного инструмента сейчас нет доступных типов торговых заявок.",
            justify="left",
        ).pack(anchor="w", pady=(0, 12))
        if error is not None:
            ttk.Label(self.content, text=str(error), justify="left", wraplength=900).pack(anchor="w", pady=(0, 12))
        ttk.Button(self.content, text="Проверить и подтвердить", state="disabled").pack(anchor="w")
    ...
    def _page_order(self: Any) -> None:
        try:
            ...
            if "нет доступных типов заявок" in str(exc):
        ...
        def walk(widget: tk.Misc) -> None:
            ...
            for child in widget.winfo_children():
                ...
                if widget_class == "TCombobox" and operation_combo is None:
                    operation_combo = child
                elif widget_class == "TButton" and str(child.cget("text")) == "Проверить и подтвердить":
                    submit_button = child
                walk(child)
