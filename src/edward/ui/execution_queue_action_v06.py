...
def can_enqueue_opportunity(result: Any) -> bool:
    """A result can enter Execution Center only when it is explicitly execution-ready."""
    decision = str(getattr(result, "decision", "") or "").upper()
    return bool(getattr(result, "execution_ready", False)) and decision in {"BUY", "ADD", "HOLD", "REDUCE", "SELL"} and int(getattr(result, "recommended_quantity", 0) or 0) > 0
...
def show_enqueue_result(result: Any) -> None:
    if getattr(result, "accepted", False):
        ...
        return
    messagebox.showwarning("Центр исполнения", getattr(result, "reason", "Решение заблокировано и не добавлено в очередь."))


class ExecutionQueueActionController:
    """Small UI-facing facade used by Opportunities and Execution Center."""
...
__all__ = ["ExecutionQueueActionController", "can_enqueue_opportunity", "enqueue_opportunity_result", "enqueue_button_text", "show_enqueue_result"]
