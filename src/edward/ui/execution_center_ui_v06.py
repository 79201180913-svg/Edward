...
_STATUS_LABELS = {
    ExecutionStatus.CREATED: "Создано", ExecutionStatus.VALIDATING: "Проверка", ExecutionStatus.READY: "Готово к исполнению",
    ExecutionStatus.WAITING_CONFIRMATION: "Ожидает подтверждения", ExecutionStatus.SUBMITTING: "Отправка заявки",
    ExecutionStatus.SUBMITTED: "Заявка отправлена", ExecutionStatus.PARTIALLY_FILLED: "Частично исполнено",
    ExecutionStatus.FILLED: "Исполнено", ExecutionStatus.RECONCILED: "Сверено", ExecutionStatus.BLOCKED: "Заблокировано",
    ExecutionStatus.REJECTED: "Отклонено", ExecutionStatus.CANCELLED: "Отменено", ExecutionStatus.TIMEOUT: "Тайм-аут",
    ExecutionStatus.FAILED: "Ошибка", ExecutionStatus.RECONCILIATION_ERROR: "Ошибка сверки",
}
_EVENT_LABELS = {
    ExecutionEventType.CREATED: "Исполнение создано", ExecutionEventType.VALIDATION_STARTED: "Начата проверка исполнения",
    ExecutionEventType.VALIDATION_PASSED: "Проверка исполнения пройдена", ExecutionEventType.VALIDATION_FAILED: "Исполнение заблокировано",
    ExecutionEventType.REVALIDATION_STARTED: "Начата повторная проверка", ExecutionEventType.REVALIDATION_FAILED: "Повторная проверка не пройдена",
    ExecutionEventType.CONFIRMATION_REQUIRED: "Требуется подтверждение пользователя", ExecutionEventType.CONFIRMED: "Пользователь подтвердил исполнение",
    ExecutionEventType.SUBMITTING: "Отправка заявки", ExecutionEventType.SUBMITTED: "Заявка отправлена",
    ExecutionEventType.STATUS_CHANGED: "Получен новый статус заявки", ExecutionEventType.FILL_UPDATED: "Обновлено исполнение",
    ExecutionEventType.CANCEL_REQUESTED: "Запрошена отмена заявки", ExecutionEventType.CANCELLED: "Заявка отменена",
    ExecutionEventType.RECONCILIATION_STARTED: "Начата сверка позиции", ExecutionEventType.RECONCILED: "Сверка завершена",
...
def build_execution_center_snapshot(*, account_label: str, service_status: str, mode_label: str, queue: list[dict[str, Any]], active: dict[str, Any] | None, events: list[Any]) -> dict[str, Any]:
    return {
        "account_label": account_label, "service_status": service_status, "mode_label": mode_label, "queue": queue,
        "active_status": execution_status_label(active.get("status")) if active else "Нет активной операции",
        "events": [{"time": getattr(e, "created_at", None), "text": execution_event_text(e), "status": execution_status_label(getattr(e, "status", "")), "message": getattr(e, "message", "")} for e in events],
    }
...
def _open_execution_center(self: Any) -> None:
    ...
    mode_var = tk.StringVar(value="Требуется подтверждение"); status_var = tk.StringVar(value="Сервис: не подключён"); active_var = tk.StringVar(value="Нет активной операции")
    ...
    prepare_button = ttk.Button(actions, text="Подготовить"); confirm_request_button = ttk.Button(actions, text="Запросить подтверждение"); confirm_submit_button = ttk.Button(actions, text="Подтвердить и отправить"); cancel_button = ttk.Button(actions, text="Отменить")
    ...
    def redraw(state: Any) -> None:
        ...
        if not items and request is not None:
            tree.insert("", "end", iid=request.execution_id, values=(request.ticker, request.decision.value, str(request.quantity), str(request.entry_price or "—"), "ДА" if request.execution_ready else "НЕТ", execution_status_label(state.status or ExecutionStatus.CREATED)))
        else:
            for item in items:
                req = item.request
                tree.insert("", "end", iid=req.execution_id, values=(req.ticker, req.decision.value, str(req.quantity), str(req.entry_price or "—"), "ДА" if req.execution_ready else "НЕТ", execution_status_label(item.result.status)))
