...
class AutonomousProtectionService:
    """Create and verify protective stop immediately after a new long position is filled."""
    ...
    def protect_fill(self, *, account_id: str, instrument_uid: str, quantity: int, result: Any) -> ProtectionResult:
        ...
        for item in self._items(active):
            ...
            if item_uid == instrument_uid and str(item.get("status", "")).upper() not in {"CANCELLED", "EXECUTED", "EXPIRED"}:
    ...
    def _items(value: Any) -> tuple[dict[str, Any], ...]:
        ...
        if isinstance(value, dict):
            value = value.get("stop_orders", value.get("orders", ()))
    ...
    def _contains(cls, value: Any, instrument_uid: str, stop_order_id: str | None) -> bool:
        for item in cls._items(value):
            uid = str(item.get("instrument_uid", item.get("instrument_id", "")))
            oid = str(item.get("stop_order_id", item.get("order_id", "")))
            if uid == instrument_uid and (not stop_order_id or oid == stop_order_id):
                return True
        return False
    ...
    def _id(value: Any) -> str | None:
        if isinstance(value, dict):
            return str(value.get("stop_order_id", value.get("order_id", ""))) or None
