...
class CurrencyService:
    ...
    def convert(self, amount: Decimal | int | float, source: str, target: str) -> Decimal:
        ...
        if source == 'USD' and target == 'RUB':
            return amount * rate
        if source == 'RUB' and target == 'USD':
            return amount / rate
    ...
    def _load_usd_rub(self) -> Decimal:
        ...
        for query in ('USD/RUB', 'USD000UTSTOM', 'USD000UTS'):
            try:
                response = self.client.find_instrument(query, trade_available_only=False)
                items = response.get('instruments', []) if isinstance(response, dict) else []
                for item in items:
                    uid = item.get('uid', item.get('instrument_uid', '')) if isinstance(item, dict) else getattr(item, 'uid', '')
                    if not uid:
                        continue
                    prices = self.client.get_last_prices([str(uid)])
                    last = prices.get('last_prices', []) if isinstance(prices, dict) else []
                    if not last:
                        continue
                    price = last[0].get('price') if isinstance(last[0], dict) else getattr(last[0], 'price', None)
                    value = self._decimal(price)
                    if value > 0:
                        self._rates[('USD','RUB')] = value
                        return value
            except Exception:
                continue
