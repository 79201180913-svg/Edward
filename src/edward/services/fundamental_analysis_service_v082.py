...
class FundamentalAnalysisServiceV082:
    """Structured fundamental analysis over the existing v0.8.1 mapped data."""
    ...
    STRATEGY_WEIGHTS = {
        "long_term": {
            "business_quality": 0.25, "growth": 0.20, "cash_generation": 0.10,
            "financial_health": 0.20, "valuation": 0.20,
            "shareholder_return": 0.05, "fundamental_momentum": 0.00,
        },
        "medium_term": {
            "business_quality": 0.15, "growth": 0.15, "cash_generation": 0.05,
            "financial_health": 0.10, "valuation": 0.10,
            "shareholder_return": 0.05, "fundamental_momentum": 0.40,
        },
        "speculative": {
            "business_quality": 0.05, "growth": 0.05, "cash_generation": 0.05,
            "financial_health": 0.10, "valuation": 0.05,
            "shareholder_return": 0.05, "fundamental_momentum": 0.65,
        },
    }

    PROFILE_ALIASES = {
        "long": "long_term", "longterm": "long_term", "long-term": "long_term",
        "medium": "medium_term", "medium-term": "medium_term", "swing": "medium_term",
        "short": "speculative", "short_term": "speculative", "short-term": "speculative",
    }
    ...
    def _metric_score(cls, metric: str, value: float | None) -> float:
        ...
        if metric in {"pe", "ps", "pb", "p_fcf", "ev_to_ebitda", "ev_to_sales"}:
            return engine.valuation(value)
        if metric == "net_debt_to_ebitda":
            return engine.leverage(value, scale=18.0)
        if metric == "total_debt_to_ebitda":
            return engine.leverage(value, scale=18.0)
        if metric == "total_debt_to_equity":
            return engine.debt_to_equity(value)
        if metric == "current_ratio":
            return engine.current_ratio(value)
        if metric == "dividend_payout":
            return engine.payout(value)
        if metric == "free_cash_flow":
            return engine.cash_flow(value)
        if metric == "free_cash_flow_to_price":
            return engine.growth(value)
        if metric == "dividend_regularity":
            return engine.clamp(value)
        return engine.growth(value)
    ...
    def _metric(cls, snapshot: Mapping[str, Any], metric: str) -> FundamentalMetricResult:
        ...
        if value is None:
            return FundamentalMetricResult(metric, None, 0.0, False, 0.0, reason_codes=("METRIC_UNAVAILABLE",))
    ...
    def _group(cls, snapshot: Mapping[str, Any], name: str) -> FundamentalGroupResult:
        ...
        if name == "business_quality":
            roe = cls._num(snapshot, "roe")
            debt_to_equity = cls._num(snapshot, "total_debt_to_equity")
    ...
    def _momentum(cls, snapshot: Mapping[str, Any]) -> FundamentalGroupResult:
        ...
        growth_5y = cls._num(snapshot, "revenue_growth_5y")
        growth_3y = cls._num(snapshot, "revenue_growth_3y")
        growth_1y = cls._num(snapshot, "revenue_growth")
        acceleration = FundamentalScoringEngineV082.growth_acceleration(growth_5y, growth_3y, growth_1y)
        earnings = {
            metric: cls._metric_score(metric, cls._num(snapshot, metric))
            for metric in ("eps_growth", "ebitda_growth")
            if cls._num(snapshot, metric) is not None
        }
        score = FundamentalScoringEngineV082.momentum(
            growth_5y=growth_5y, growth_3y=growth_3y, growth_1y=growth_1y,
    ...
    def _weighted_overall(cls, groups: tuple[FundamentalGroupResult, ...], profile: str):
        ...
        if not usable:
            return 0.0, 0.0, tuple(defaults.items())
        total = sum(defaults[group.name] for group in usable)
        normalized = tuple((group.name, defaults[group.name] / total) for group in usable)
    ...
    def analyze(cls, fundamentals: Any = None, *, profile: str = "medium_term") -> FundamentalAnalysisResult:
        ...
        if not isinstance(fundamentals, Mapping) or not fundamentals:
            ...
            return FundamentalAnalysisResult(
                *(empty[name] for name in cls.GROUPS), 0.0, 0.0, 0.0, "UNAVAILABLE",
                selected_profile, tuple(cls.STRATEGY_WEIGHTS[selected_profile].items()), ("NO_FUNDAMENTAL_DATA",)
            )

        groups = (
            cls._group(fundamentals, "business_quality"), cls._group(fundamentals, "growth"),
            cls._group(fundamentals, "cash_generation"), cls._group(fundamentals, "financial_health"),
            cls._group(fundamentals, "valuation"), cls._group(fundamentals, "shareholder_return"),
