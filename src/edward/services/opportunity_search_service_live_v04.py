...
from edward.services.application_settings import ApplicationSettingsStore
...
class LiveOpportunitySearchService(OpportunitySearchService):
    ...
    def _canonical_result(self, instrument: Any, opportunity: Any, quantity: float) -> OpportunitySearchResult:
        ...
        if decision == "pass":
            if status == "rejected":
                reason = "TRADING_PATH_REJECTED"
            elif not bool(path_opportunity.risk_gate):
                reason = "RISK_GATE_FAILED"
            elif path_opportunity.score is None:
                reason = "OPPORTUNITY_SCORE_UNAVAILABLE"
            else:
                reason = "NO_PROMOTED_TRADING_PATH"
        elif decision == "wait":
            reason = "TRADING_PATH_WAIT"
        elif decision == "buy":
            reason = "TRADING_PATH_BUY"
        explanation = (
            f"Trading Path: {path.hypothesis}; regime={path.regime}; volatility={path.volatility_bucket}; "
            f"direction={path.direction}; horizon={path.horizon}; status={status}; state={state}."
        )
    ...
    def scan(
        ...
        *,
        ...
        for instrument in instruments:
            ...
            try:
                ...
                if len(candles) < 300:
                    ...
                    canonical = self.path_runtime.scan_instrument(
                        instrument_uid=uid,
                        ticker=ticker,
                        candles=candles,
                        profile=profile,
                ...
                opportunity_search_module.logger.exception("[V013 OPPORTUNITY ERROR] uid=%s ticker=%s", uid, ticker)
            ...
            if result_callback is not None:
                try:
                    ...
                    opportunity_search_module.logger.exception("[V013 OPPORTUNITY CALLBACK] ticker=%s", ticker)
        ...
        results.sort(key=lambda item: (item.decision not in {"BUY", "WAIT"}, -item.opportunity_score))
    ...
    def _unavailable(instrument: Any, price: float | None, quantity: float, reason: str) -> OpportunitySearchResult:
        return OpportunitySearchResult(
            str(LiveOpportunitySearchService._field(instrument, "uid", LiveOpportunitySearchService._field(instrument, "instrument_uid", ""))),
            str(LiveOpportunitySearchService._field(instrument, "ticker", "")),
            str(LiveOpportunitySearchService._field(instrument, "name", "")),
            price,
            None,
            None,
            0.0,
