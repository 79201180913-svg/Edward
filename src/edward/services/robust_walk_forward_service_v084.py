...
class RobustWalkForwardServiceV084(RobustWalkForwardService):
    ...
    def _select_robust_parameters(cls, candidates: Sequence[tuple[dict[str, Any], ResearchBacktestResult]]) -> tuple[dict[str, Any], ResearchBacktestResult, float]:
        ...
        logger.warning("[V084 WF VIABILITY FILTER] candidates=%d max_train_dd=%s min_train_trades=%d", len(candidates), max_drawdown_pct, min_trades)
        for params, result in candidates:
            decision = EconomicViabilityServiceV084.evaluate(result, min_excess_return_pct=0.0, max_drawdown_pct=max_drawdown_pct, min_trades=min_trades)
    ...
    def run(cls, *, candles, strategy: str, parameter_grid, signal_factory, train_size: int, test_size: int, costs: BacktestCostModel | None = None, max_drawdown_pct: float | None = None, min_train_trades: int = 1):
        ...
        try:
            ...
            logger.warning("[V084 WF RESULT] strategy=%s windows=%d robustness=%.2f mean_oos_return=%.4f mean_oos_dd=%.4f mean_oos_sharpe=%.4f", strategy, len(result.windows), result.robustness_score, result.mean_test_return_pct, result.mean_test_drawdown_pct, result.mean_test_sharpe)
            return result
        finally:
