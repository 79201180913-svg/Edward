...
class RobustWalkForwardResultV084(RobustWalkForwardResult):
    """v0.8.4 result carrying per-window Train-only parameter-zone evidence."""

    parameter_zone_diagnostics: tuple[ParameterZoneV084, ...] = ()


class RobustWalkForwardServiceV084(RobustWalkForwardService):
    """v0.8.4 WF selector with Train-only economic viability and zone diagnostics."""
    ...
    @staticmethod
    def _with_zones(result: RobustWalkForwardResult, zones: Sequence[ParameterZoneV084]) -> RobustWalkForwardResultV084:
        return RobustWalkForwardResultV084(
            strategy=result.strategy,
            windows=result.windows,
            mean_test_return_pct=result.mean_test_return_pct,
            median_test_return_pct=result.median_test_return_pct,
            std_test_return_pct=result.std_test_return_pct,
            worst_test_return_pct=result.worst_test_return_pct,
            best_test_return_pct=result.best_test_return_pct,
            mean_test_drawdown_pct=result.mean_test_drawdown_pct,
            ...
            return_consistency_pct=result.return_consistency_pct,
            risk_consistency_pct=result.risk_consistency_pct,
            sharpe_consistency_pct=result.sharpe_consistency_pct,
            robustness_score=result.robustness_score,
            parameter_stability=result.parameter_stability,
            version=result.version,
            parameter_zone_diagnostics=tuple(zones),
        )
    ...
    def run(cls, *, candles, strategy: str, parameter_grid, signal_factory, train_size: int, test_size: int, costs: BacktestCostModel | None = None, max_drawdown_pct: float | None = None, min_train_trades: int = 1):
        ...
        try:
            ...
            result_v084 = cls._with_zones(result, zones)
