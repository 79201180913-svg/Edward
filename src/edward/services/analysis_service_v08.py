...
class AnalysisServiceV08:
    ...
    def analyze(self, *, instrument_uid: str, ticker: str, candles: Iterable[Candle], profile: str = "medium_term", risk_profile: str = "balanced", horizon: str = "medium") -> AnalysisResult:
        ...
        strategies = [self._legacy_strategy_result(item, profile) for item in robust_results]; passed = [item for item in strategies if item.quality_gate]; winner = max(passed, key=lambda item: item.score) if passed else None; score_winner = max(strategies, key=lambda item: item.score) if strategies else None
