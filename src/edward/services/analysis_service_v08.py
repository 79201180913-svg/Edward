...
class AnalysisV08Diagnostics:
    ...
    trading_path_candidates: tuple[TradingPathCandidate, ...] = ()
...
class AnalysisServiceV08:
    ...
    def __init__(self, *, costs: BacktestCostModel | None = None) -> None:
        ...
        logger.warning(
            "[V087 EXEC] INIT AnalysisServiceV08 file=%s version=%s legacy_result_version=%s strategies=%s profiles=%s discovery=%s conditional=%s",
            __file__,
            ANALYSIS_V08_VERSION,
            LEGACY_ANALYSIS_RESULT_VERSION,
            self.STRATEGIES,
            self.PROFILES,
            ResearchDiscoveryServiceV085.HYPOTHESES,
            ConditionalDiscoveryServiceV086.HYPOTHESES,
        )
    ...
    def _robust(self, candles: list[Candle], strategy: str, profile: str) -> RobustWalkForwardResult:
        ...
        logger.warning(
            "[V083 ROBUSTNESS BREAKDOWN] strategy=%s return_score=%.2f return_contribution=%.2f risk_score=%.2f risk_contribution=%.2f sharpe_score=%.2f sharpe_contribution=%.2f stability_score=%.2f stability_contribution=%.2f performance_score=%.2f performance_contribution=%.2f total=%.2f",
            strategy,
            diagnostics.return_consistency_score,
            diagnostics.return_contribution,
            diagnostics.risk_consistency_score,
            diagnostics.risk_contribution,
            diagnostics.sharpe_consistency_score,
            diagnostics.sharpe_contribution,
            diagnostics.parameter_stability_score,
            diagnostics.parameter_stability_contribution,
            diagnostics.performance_consistency_score,
            diagnostics.performance_consistency_contribution,
            diagnostics.robustness_total,
        )
        logger.warning(
            "[V083 ROBUSTNESS ACTIVITY] strategy=%s windows=%d active=%d inactive=%d active_pct=%.2f positive_active=%d positive_active_pct=%.2f positive_all_pct=%.2f",
            strategy,
            diagnostics.total_windows,
            diagnostics.active_windows,
            diagnostics.inactive_windows,
            diagnostics.active_pct,
            diagnostics.positive_active_windows,
            diagnostics.positive_active_pct,
        ...
        logger.warning(
            ...
            strategy, len(result.windows), result.robustness_score, result.mean_test_return_pct, result.mean_test_drawdown_pct, result.mean_test_sharpe,
        )
        return result
    ...
    def analyze(
        ...
        *,
        ...
        logger.warning(
            "[V087 EXEC] ENTER AnalysisServiceV08 file=%s ticker=%s instrument_uid=%s profile=%s candles=%d minimum=%d",
        ...
        trading_path_candidates = TradingPathCandidateServiceV088.promote(
            conditional_discovery,
            instrument_uid=instrument_uid,
            ticker=ticker,
        )
        logger.warning(
            "[V012 PATH DISCOVERY] ticker=%s cells=%d candidates=%d",
            ticker,
            sum(len(evidence.cells) for evidence in conditional_discovery.evidence),
            len(trading_path_candidates),
        )
        for candidate in trading_path_candidates:
            logger.warning(
                "[V012 PATH CANDIDATE] ticker=%s family_hypothesis=%s regime=%s volatility=%s direction=%s horizon=%d observations=%d excess=%.6f status=%s",
                ticker,
        ...
        logger.warning(
            "[V087 RESEARCH SUMMARY] ticker=%s cells=%d interesting=%d low_sample=%d no_positive_excess=%d low_wf_persistence=%d",
            ticker, research_summary.total_cells, research_summary.interesting, research_summary.low_sample, research_summary.no_positive_excess, research_summary.low_wf_persistence,
        )
        for row in research_summary.top_magnitude:
            logger.warning(
                "[V087 RESEARCH MAGNITUDE] ticker=%s strategy=%s rank=%d hypothesis=%s regime=%s volatility=%s direction=%s horizon=%d N=%d excess=%.6f flag=%s",
                ticker, row.strategy_context, row.magnitude_rank, row.evidence.hypothesis, row.evidence.regime, row.evidence.volatility_bucket, row.evidence.direction, row.evidence.horizon, row.evidence.observations, row.evidence.excess_return_pct, row.research_flag,
            )
        for row in research_summary.top_consistency:
            logger.warning(
                "[V087 RESEARCH CONSISTENCY] ticker=%s strategy=%s rank=%d hypothesis=%s regime=%s volatility=%s direction=%s horizon=%d N=%d win_rate=%.2f flag=%s",
                ticker, row.strategy_context, row.consistency_rank, row.evidence.hypothesis, row.evidence.regime, row.evidence.volatility_bucket, row.evidence.direction, row.evidence.horizon, row.evidence.observations, row.evidence.win_rate_pct, row.research_flag,
            )
        for row in research_summary.top_stability:
            logger.warning(
                "[V087 RESEARCH STABILITY] ticker=%s strategy=%s rank=%d hypothesis=%s regime=%s volatility=%s direction=%s horizon=%d N=%d wf_persistence=%.2f flag=%s",
                ticker, row.strategy_context, row.stability_rank, row.evidence.hypothesis, row.evidence.regime, row.evidence.volatility_bucket, row.evidence.direction, row.evidence.horizon, row.evidence.observations, row.wf.wf_persistence_pct if row.wf else 0.0, row.research_flag,
            )
        ...
        self.last_diagnostics = AnalysisV08Diagnostics(
            ...
            trading_path_candidates=trading_path_candidates,
            evidence_audit=evidence_audit,
            wf_evidence=tuple(wf_evidence),
        ...
        explanation = (
            ...
            if winner
            else f"Ни одна стратегия не прошла v0.8 Quality Gate; режим {regime_result.regime}, regime confidence {regime_result.confidence:.0f}%. Исследовательские слои v0.8.5/v0.8.6/v0.8.7 дополнительно проверили структурные, условные и доказательные гипотезы без влияния на допуск к торговле."
        )
