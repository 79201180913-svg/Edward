...
class AnalysisServiceV08:
    ...
    def _grid(strategy: str, profile: str) -> list[dict[str, Any]]:
        if strategy == "Trend Following": return [{"fast": f, "slow": s} for f, s in ((10, 30), (20, 50), (30, 90))]
    ...
    def _robust(self, candles: list[Candle], strategy: str, profile: str) -> RobustWalkForwardResult:
        ...
        logger.info("[WALK FORWARD] strategy=%s profile=%s candles=%d train=%d test=%d window_size=%d available_windows=%d min_required_windows=%d parameter_candidates=%d", strategy, profile, candle_count, cfg["train"], cfg["test"], window_size, available_windows, minimum_windows, len(parameter_grid))
        if available_windows < minimum_windows:
            logger.warning("[WALK FORWARD] strategy=%s status=INSUFFICIENT_DATA available_windows=%d required_windows=%d candles=%d required_candles=%d", strategy, available_windows, minimum_windows, candle_count, required_candles)
        else:
            logger.info("[WALK FORWARD] strategy=%s status=DATA_SUFFICIENT available_windows=%d required_windows=%d", strategy, available_windows, minimum_windows)
        result = RobustWalkForwardService.run(candles=candles, strategy=strategy, parameter_grid=parameter_grid, signal_factory=self._signal_factory, train_size=cfg["train"], test_size=cfg["test"], costs=self.costs, max_drawdown_pct=cfg["max_drawdown_pct"])
        logger.info("[WALK FORWARD] strategy=%s completed_windows=%d parameter_stability=%.2f robustness=%.2f", strategy, len(result.windows), result.parameter_stability.stability_pct, result.robustness_score)
        return result
    @staticmethod
    def _quality(result: RobustWalkForwardResult, profile: str) -> bool:
        ...
        logger.info("[QUALITY GATE] strategy=%s profile=%s result=%s", result.strategy, profile, "PASS" if diagnostics.passed else "FAIL")
        for check in diagnostics.checks:
            logger.info("[QUALITY GATE] strategy=%s check=%s actual=%.4f threshold=%.4f result=%s", result.strategy, check.key, check.actual, check.threshold, "PASS" if check.passed else "FAIL")
        logger.info("[QUALITY GATE] strategy=%s failed_checks=%s reason=%s", result.strategy, diagnostics.failed_checks, diagnostics.failure_reason or "none")
    ...
    def analyze(self, *, instrument_uid: str, ticker: str, candles: Iterable[Candle], profile: str = "medium_term", risk_profile: str = "balanced", horizon: str = "medium") -> AnalysisResult:
        ...
        self.last_diagnostics = AnalysisV08Diagnostics(regime_confidence=regime_result.confidence, regime=regime_result.regime, robustness_by_strategy={item.strategy: item.robustness_score for item in robust_results}, quality_gate_by_strategy={item.strategy: QualityGateDiagnosticsServiceV0822.evaluate(item, profile) for item in robust_results})
        logger.info("[STRATEGY SELECTION] ticker=%s profile=%s quality_gate_winner=%s max_score_strategy=%s quality_gate_pass_count=%d total_strategies=%d", ticker, profile, winner.strategy if winner else None, score_winner.strategy if score_winner else None, len(passed), len(strategies))
        explanation = f"Рекомендована {winner.strategy}: v0.8 robustness {winner.score:.1f}, OOS return {winner.return_pct:.2f}%, Sharpe {winner.sharpe:.2f}, режим {regime_result.regime}, regime confidence {regime_result.confidence:.0f}%." if winner else f"Ни одна стратегия не прошла v0.8 Quality Gate; режим {regime_result.regime}, regime confidence {regime_result.confidence:.0f}%."
