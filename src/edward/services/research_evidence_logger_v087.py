...
class ResearchEvidenceLoggerV087:
    ...
    def build_and_log(self, evidence: Iterable, wf_evidence: Iterable = (), *, ticker: str, limit: int = 10) -> ResearchEvidenceSummaryV087:
        rows: tuple[ResearchEvidenceRowV086, ...] = ResearchEvidenceReportServiceV086.build(evidence, wf_evidence)
        summary = ResearchEvidenceSummaryServiceV087.build(rows, limit=limit)
        self.logger.warning(
            "[V087 RESEARCH SUMMARY] ticker=%s cells=%d interesting=%d low_sample=%d no_positive_excess=%d low_wf_persistence=%d top_magnitude=%d top_consistency=%d top_stability=%d",
        ...
        for rank, row in enumerate(summary.top_magnitude, 1):
            self.logger.warning(
                "[V087 RESEARCH MAGNITUDE] ticker=%s rank=%d hypothesis=%s regime=%s volatility=%s direction=%s horizon=%d excess=%.6f N=%d flag=%s",
                ticker, rank, row.evidence.hypothesis, row.evidence.regime,
        ...
        for rank, row in enumerate(summary.top_consistency, 1):
            self.logger.warning(
                "[V087 RESEARCH CONSISTENCY] ticker=%s rank=%d hypothesis=%s regime=%s volatility=%s direction=%s horizon=%d win_rate=%.2f N=%d flag=%s",
                ticker, rank, row.evidence.hypothesis, row.evidence.regime,
                row.evidence.volatility_bucket, row.evidence.direction, row.evidence.horizon,
        ...
        for rank, row in enumerate(summary.top_stability, 1):
            self.logger.warning(
                "[V087 RESEARCH STABILITY] ticker=%s rank=%d hypothesis=%s regime=%s volatility=%s direction=%s horizon=%d wf_persistence=%.2f wf_windows=%d N=%d flag=%s",
                ticker, rank, row.evidence.hypothesis, row.evidence.regime,
                row.evidence.volatility_bucket, row.evidence.direction, row.evidence.horizon,
