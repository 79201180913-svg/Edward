...
class ExecutionCenterController:
    ...
    def load_queue_item(self, item: Any) -> ExecutionCenterState:
        request = getattr(item, "request", None)
        result = getattr(item, "result", None)
        if request is None or result is None:
            raise ValueError("invalid execution queue item")
        if self.state.request is not None and self.state.request.execution_id != request.execution_id:
            raise ValueError("another execution is already active")
        self.state = ExecutionCenterState(request=request, result=result, status=result.status)
        self._publish()
        return self.state
