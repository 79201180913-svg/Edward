...
class TradingPathExpectedValueServiceV012:
        ...
        for offset in range(windows):
            ...
            for item in observations:
                if not (start <= item.index < end):
                    continue
                if item.hypothesis != candidate.rule.hypothesis:
                    continue
                if item.regime != candidate.rule.regime:
                    continue
                if item.volatility_bucket != candidate.rule.volatility_bucket:
                    continue
                if item.direction != candidate.rule.direction:
                    continue
    ...
    def calculate(
        ...
        *,
        ...
        values = cls.outcomes(candidate, candles, windows=windows, test_size=test_size)
        ...
        logger.warning(
            ...
            result.observations,
            result.expected_value_pct,
            result.ev_ci_low_pct,
            result.ev_ci_high_pct,
            result.edge_reliability_pct,
            result.edge_reliability_level,
        )
        return result
