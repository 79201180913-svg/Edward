...
class RobustWalkForwardService:
    ...
    def run(cls, *, candles: Iterable[Candle], strategy: str, parameter_grid: Sequence[dict[str, Any]], signal_factory: Callable[[str, dict[str, Any]], Callable[[Sequence[Candle], int], bool]], train_size: int, test_size: int, costs: BacktestCostModel | None = None, max_drawdown_pct: float | None = None) -> RobustWalkForwardResult:
        ...
        for criterion in criterion_matches:
            gaps=criterion_gaps[criterion]; logger.warning("[V083 WF SELECTION RESULT] strategy=%s criterion=%s matched_windows=%d match_pct=%.2f mean_selection_gap=%.4f",strategy,criterion,criterion_matches[criterion],criterion_matches[criterion]/count*100,mean(gaps) if gaps else 0)
        logger.warning("[V083 WF STABILITY RESULT] strategy=%s windows=%d mean_winner_margin=%.2f mean_neighborhood=%.2f mean_selection_confidence=%.2f",strategy,count,mean(stability_margins) if stability_margins else 0,mean(stability_neighborhoods) if stability_neighborhoods else 0,mean(stability_confidences) if stability_confidences else 0)
        return result
