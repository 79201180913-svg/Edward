...
def _oos_returns(oos_results: Sequence[object]) -> tuple[float, ...]:
    ...
    for window in oos_results:
        ...
        if returns is not None:
            values.extend(float(value) for value in returns)
            continue
        observations = int(_field(window, "observations") or 0)
        mean_return = _field(window, "mean_return_pct")
        if observations > 0 and mean_return is not None:
            values.extend([float(mean_return)] * observations)
    return tuple(values)
...
class AnalysisPathRuntimeServiceV012:
    ...
    def _candidate_key(candidate: object) -> tuple[object, ...]:
        rule = candidate.rule
        return (rule.instrument_uid, rule.ticker, rule.hypothesis, rule.regime, rule.volatility_bucket, rule.direction, rule.horizon)

    @staticmethod
    def _analysis_key(analysis: TradingPathAnalysisV012) -> tuple[object, ...]:
        return (analysis.instrument_uid, analysis.ticker, analysis.hypothesis, analysis.regime, analysis.volatility_bucket, analysis.direction, analysis.horizon)
    ...
    def analyze_paths(self, *, instrument_uid: str, ticker: str, candles: Iterable[Candle], profile: str = "medium_term", risk_profile: str = "balanced", benchmark_candles: Iterable[Candle] | None = None, benchmark_id: str | None = None) -> tuple[TradingPathAnalysisV012, ...]:
        ...
        for analysis in selected:
            ...
            ev_evidence = TradingPathEVEvidenceServiceV015.build(_oos_returns(oos_results))
            ...
            if quality_gate.passed:
                ...
                final_decision = TradingPathDecision(decision_result.decision.value)
            ...
            logger.warning("[V015 PATH DECISION] ticker=%s hypothesis=%s regime=%s volatility=%s direction=%s horizon=%d rank=%s validation=%s ev=%s risk=%s opportunity=%s confidence=%s decision=%s state=%s reason=%s wf_persistence=%s market_context=%s oos_evidence=%s ev_evidence=%s quality_gate=%s quality_reasons=%s", ticker, final.hypothesis, final.regime, final.volatility_bucket, final.direction, final.horizon, final.rank, _value(final.status), _field(opportunity, "expected_value_pct"), _field(opportunity, "risk_score"), _field(opportunity, "score"), _field(opportunity, "confidence"), _value(final.decision), _value(final.current_state), ",".join(quality_gate.reasons) or "READY", _field(final.validation, "wf_persistence_pct"), final.market_context.context_status, independent_oos_evidence.status, ev_evidence.status, quality_gate.passed, ",".join(quality_gate.reasons) or "READY")
