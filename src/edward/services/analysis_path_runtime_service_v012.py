...
class AnalysisPathRuntimeServiceV012:
    ...
    def analyze_paths(self, *, instrument_uid: str, ticker: str, candles: Iterable[Candle], profile: str = "medium_term", risk_profile: str = "balanced", benchmark_candles: Iterable[Candle] | None = None, benchmark_id: str | None = None, context: TradingPathContextV015 | None = None) -> tuple[TradingPathAnalysisV012, ...]:
        ...
        logger.warning("[V015 PATH RUNTIME] ticker=%s candles=%d train=%d validation=%d oos=%d discovered=%d selected=%d final=%d buy=%d wait=%d pass=%d nested_folds=%d nested_candidates=%d", ticker, len(ordered), len(train), len(validation_candles), len(oos), len(candidates), len(selected), len(finalized), sum(_value(item.decision) == "buy" for item in finalized), sum(_value(item.decision) == "wait" for item in finalized), sum(_value(item.decision) == "pass" for item in finalized), len(nested.folds), sum(fold.discovered_candidates for fold in nested.folds))
