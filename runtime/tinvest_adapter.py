...
def _configure_windows_ca_bundle() -> None:
    ...
    with path.open("w", encoding="ascii") as fh:
        for cert in certs:
            ...
            for i in range(0, len(encoded), 64): fh.write(encoded[i:i + 64] + "\n")
...
def _protobuf_to_dict(value: Any) -> Any:
    ...
    if isinstance(value, Message): return MessageToDict(value, preserving_proto_field_name=True)
    if isinstance(value, bytes): return value.decode("utf-8", errors="replace")
    if isinstance(value, dict): return {str(k): _protobuf_to_dict(v) for k, v in value.items()}
    if isinstance(value, (list, tuple)): return [_protobuf_to_dict(v) for v in value]
    for attr in ("response", "result", "data"):
        nested = getattr(value, attr, None)
        if nested is not None and nested is not value:
            try: return _protobuf_to_dict(nested)
            except Exception: pass
    result: dict[str, Any] = {}
    for name in dir(value):
        if name.startswith("_"): continue
        try: attr = getattr(value, name)
        except Exception: continue
        if callable(attr): continue
        if isinstance(attr, (str, int, float, bool, type(None), bytes, list, tuple, dict)): result[name] = _protobuf_to_dict(attr)
...
def _camel_to_snake(value: Any) -> Any:
    if isinstance(value, list): return [_camel_to_snake(v) for v in value]
...
class AdapterState:
    def __init__(self) -> None:
        if not TOKEN: raise RuntimeError("T-Invest API token is not configured")
    ...
    def close(self) -> None: self.client.__exit__(None, None, None)

    def _service(self, name: str) -> Any:
        service = getattr(self.client, name, None)
        if service is None: raise RuntimeError(f"T-Invest SDK service '{name}' is unavailable")
        return service

    def _rest_request(self, method: str, payload: dict[str, Any]) -> dict[str, Any]:
        request = Request(f"{REST_TARGET}/rest/tinkoff.public.invest.api.contract.v1.{method}", data=json.dumps(payload, ensure_ascii=False).encode("utf-8"), method="POST", headers={"Authorization": f"Bearer {TOKEN}", "Content-Type": "application/json", "Accept": "application/json"})
        try:
            with urlopen(request, timeout=30.0) as response: return _camel_to_snake(json.loads(response.read().decode("utf-8")))
        except HTTPError as exc:
            try: body = json.loads(exc.read().decode("utf-8"))
            except Exception: body = {"error": "http_error", "message": str(exc)}
            raise RuntimeError(body.get("message", body.get("error", str(exc)))) from exc
        except URLError as exc: raise RuntimeError(f"T-Invest REST API is unavailable: {exc.reason}") from exc
    ...
    def last_prices(self, ids): return self._service("market_data").get_last_prices(instrument_id=ids)
    def trading_status(self, instrument_id): return self._service("market_data").get_trading_status(instrument_id=instrument_id)
    def trading_statuses(self, ids): return self._service("market_data").get_trading_statuses(instrument_id=ids)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send(self,status,payload):
        body=json.dumps(payload,ensure_ascii=False,separators=(",",":")).encode("utf-8"); self.send_response(status); self.send_header("Content-Type","application/json; charset=utf-8"); self.send_header("Content-Length",str(len(body))); self.end_headers(); self.wfile.write(body)
    def _read_json(self):
    ...
    def do_POST(self):
        try:
            ...
            if self.path=="/market/last-prices": self._send(200,message_to_dict(STATE.last_prices([str(x) for x in p.get("instrument_ids",[])]))); return
            ...
            if self.path=="/orders/price": self._send(200,message_to_dict(STATE.order_price({"accountId":p["account_id"],"instrumentId":p["instrument_id"],"price":p["price"],"direction":p["direction"],"quantity":str(p["quantity"])}))); return
            if self.path=="/orders/max-lots": self._send(200,message_to_dict(STATE.max_lots({"accountId":p["account_id"],"instrumentId":p["instrument_id"],"price":p["price"]}))); return
            if self.path=="/operations": self._send(200,message_to_dict(STATE.operations(str(p.get("account_id","")).strip(),int(p.get("limit",1000))))); return
            ...
            self._send(404,{"error":"not_found"})
        except Exception as exc: self._send(502,{"error":type(exc).__name__,"message":str(exc)})

def main():
    server=ThreadingHTTPServer((HOST,PORT),Handler); print(f"Edward T-Invest adapter started on http://{HOST}:{PORT}",flush=True)
