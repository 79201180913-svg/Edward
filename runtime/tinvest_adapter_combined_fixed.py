...
def _operations(self, account_id, limit=1000):
    ...
    if _adapter.ENVIRONMENT != "sandbox":
        return _adapter.message_to_dict(self._service("operations").get_operations_by_cursor(
            account_id=str(account_id), limit=limit, without_commissions=False, without_trades=False))
    return self._rest_request("SandboxService/GetSandboxOperationsByCursor", {
        "accountId": str(account_id), "limit": limit,
...
def _order_state(self, account_id, order_id):
    if _adapter.ENVIRONMENT != "sandbox":
        return _adapter.message_to_dict(self._service("orders").get_order_state(
            account_id=str(account_id), order_id=str(order_id)))
    return self._rest_request("SandboxService/GetSandboxOrderState", {
        "accountId": str(account_id), "orderId": str(order_id),
...
def _create_order(self, payload):
    if _adapter.ENVIRONMENT != "sandbox":
        return _adapter.message_to_dict(self._service("orders").post_order(
            quantity=int(payload["quantity"]),
            direction=fixed._sdk_order_direction(payload["direction"]),
            account_id=str(payload["account_id"]),
            order_type=fixed._sdk_order_type(payload["order_type"]),
            instrument_id=str(payload["instrument_uid"]),
            order_id=str(payload.get("request_id") or payload.get("order_id") or uuid.uuid4()),
            price=_adapter._sdk_quotation(payload["price"]) if payload.get("price") is not None else None))
    ...
    if not instrument_id:
        raise ValueError("instrument_uid/instrument_id is required")
    request = {
        "quantity": str(int(payload["quantity"])),
        "direction": direction,
        "accountId": str(payload["account_id"]),
        "orderType": order_type,
        "orderId": str(payload.get("request_id") or payload.get("order_id") or uuid.uuid4()),
        "instrumentId": instrument_id,
        "timeInForce": str(payload.get("time_in_force") or "TIME_IN_FORCE_DAY"),
...
def _cancel_order(self, account_id, order_id):
    if _adapter.ENVIRONMENT != "sandbox":
        return _adapter.message_to_dict(self._service("orders").cancel_order(
            account_id=str(account_id), order_id=str(order_id)))
    return self._rest_request("SandboxService/CancelSandboxOrder", {
        "accountId": str(account_id), "orderId": str(order_id),
        "orderIdType": "ORDER_ID_TYPE_UNSPECIFIED"})
...
def _replace_order(self, payload):
    ...
    if not order_id or not account_id or quantity <= 0:
        raise ValueError("account_id, order_id and positive quantity are required")
    if _adapter.ENVIRONMENT != "sandbox":
